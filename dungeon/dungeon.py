...
class Dungeon:
    ...
    def __init__(self, N):
        ...
        self.dungeon[0, :] = self.dungeon[-1, :] = self.dungeon[:, 0] = self.dungeon[:, -1] = Constants.OBSTACLE
        
        # Pick N/2 random coordinates for the obstacles
        empty_coord = self.get_empty_cells( int(self.size/2) )
        self.dungeon[empty_coord[0], empty_coord[1]] = Constants.OBSTACLE
        
        # Pick N/2 random coordinates for the lava
        empty_coord = self.get_empty_cells( int(self.size/2) )
        self.dungeon[empty_coord[0], empty_coord[1]] = Constants.LAVA
        ...
        self.dungeon[self.position_exit[0], self.position_exit[1]] = Constants.EXIT
    ...
    def step(self, action):
        ...
        reward = Constants.TIMESTEP_REWARD
        ...
        # If the agent bumps into a wall, it doesn't move but you still get a negative reward
        if self.dungeon[next_position[0], next_position[1]] == Constants.OBSTACLE:
            reward += Constants.OBSTACLE_REWARD
        else:
            self.position_agent = next_position
        
        # calculate reward
        current_cell_type = self.dungeon[self.position_agent[0], self.position_agent[1]]
        if current_cell_type == Constants.LAVA:
            reward += Constants.LAVA_REWARD
        
        if current_cell_type == Constants.EXIT:
            reward += self.size**2
